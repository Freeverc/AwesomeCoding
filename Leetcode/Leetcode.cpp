...
public:
    ...
    int get(int key) {
        if (listMap.find(key) != listMap.end()) {
            if (listMap[key]->last && listMap[key]->next) {
                listMap[key]->last->next = listMap[key]->next;
                listMap[key]->next->last = listMap[key]->last;
                tail->next = listMap[key];
                listMap[key]->last = tail;
                tail = listMap[key];
                tail->next = NULL;
            }
            else if (!listMap[key]->last && listMap[key]->next) {
                head = head->next;
                head->last = NULL;
                tail->next = listMap[key];
                listMap[key]->last = tail;
    ...
    void put(int key, int value) {
        if (listMap.find(key) != listMap.end()) {
            ...
            listMap[key] = new Node();
            listMap[key]->key = key;
            listMap[key]->value = value;
            if (head == NULL) {
				listMap[key]->last = NULL;
				listMap[key]->next = NULL;
                head = tail = listMap[key];
                size++;
            }
            else {
				tail->next = listMap[key];
...
public:
    ...
    void push(int x) {
        values.push_back(x);
        topIndex++;
        if (minIndex == -1 || values[minIndex] > x)
            minIndex = topIndex;
    }
        ...
        if (topIndex >= 0) {
            ...
            if (minIndex == topIndex) {
				...
				for (int i = 0; i < topIndex; i++) {
					if (minIndex == -1 || values[minIndex] > values[i])
						minIndex = i;
				}
            }
			topIndex--;
    ...
    int top() {
        return topIndex>=0? values[topIndex]:0;
    }

    int getMin() {
        return minIndex>=0? values[minIndex]:0;
    }
};
...
public:
    ...
    bool startsWith(string prefix) {
        ...
        int find = false;
        for (auto root : *curr) {
            if (root->val == prefix[0]) {
                if (prefix.size() > 1) {
                    curr = &root->nexts;
                    find = startsWith(prefix.substr(1));
                }
                else {
                    curr = &roots;
                    find = true;
...
{
    Solution solution;
    //vector<int>  data = {0, 0, 0, 0, 5,-1,0, 9, -6,0,0,0,  1, 0, -1, 0, 3,0,0,0, 90, -90, 2};
    //vector<int>  data = {1,-1,1,-1,9,-3,3,-8,0,-1, 0};
    //vector<int>  data = {0, 0};
    //vector<int>  data = {0,0,0, 10,-7,13,-14,30,16,0,0,0,-3,-16,-27,27,19,0,0,0,0 };
    //int target = 9;
    //vector<int> result = s.twoSum2(data, target);
    //bool r = s.canThreePartsEqualSum3(data);
    //cout << r << endl;
    //ListNode l1(2);
    //l1 = NULL;
    ...
    //ListNode l3(3);
    //vector<ListNode*> v = { &l2,lp1, &l1 , &l3};
    //ListNode* r = solution.mergeKLists2(v);
    //while (r != NULL)
    //{
    //    cout << r->val << endl;
    //    r = r->next;
    //}
	 ...
	 //   time_t tStart, tEnd;
		////string s = "bbbbb";
		//string s = "pwwkuiomeweytw";
	 //   tStart = clock();
	 //   while (n--)
	 //   {
		//	int l = solution.lengthOfLongestSubstring2(s);
	 //   }
	 //   tEnd = clock();
    ...
    //cout << solution.isMatch2(s, p) << endl;
    //string s = "ip";
    //string p = "p*.";
    ...
    //   vector<vector<int>> r = solution.threeSum3(nums);
    //vector<vector<int>>::iterator iter1 = r.begin();
    //vector<int>::iterator iter2;
    //string d = "235476848342";
    //vector<string> r = solution.letterCombinations(d);
    ...
    //for(int i = 0;i<nums.size();i++)
        //cout << nums[i] <<" ";
    //string s = "cabdidcab";
    //vector<string> wd = {"c", "cab", "did"};
    //cout << solution.wordBreak(s, wd) << endl;
    //int a = 3 > 2 ? 3 : 2;
    //cout << a;
    //vector<int> nums = {7,6,5,3,2,1};
    ...
    //        cout << r[i][j] << " ";
    //    cout <<" " <<solution.ks[i]<< endl;
    //}
    //cout << accumulate(solution.ks.begin(), solution.ks.end(), 0)/solution.ks.size() << endl;
    //vector<int> nums = { -2,1,-3,4,-1,2,1,-5,4 };
    //cout << solution.maxSubArray(nums);
    //cout << solution.uniquePaths(19, 6) << endl;
    //cout << solution.minDistance("horse", "ros") << endl;
    //vector<int> nums = { 1,  2, 2, 3, 3,5,5};
    //vector<int> r =solution.searchRange(nums, 0);
    //cout << r[0] << endl;
    //cout << r[1] << endl;
    //vector<int> nums = { 2, 3, 4, 5};
    //int target = 1;
    //vector<vector<int> > results = solution.combinationSum(nums, target);
    ...
    //    cout << endl;
    //}
    //vector<char> board0 = { 'A','B','C','E' };
    //vector<char> board1 = { 'S','F','C','S' };
    //vector<char> board2 = { 'A','D','E','E' };
    //vector<vector<char>> board = {board0, board1, board2};
    //bool r =solution.exist1(board, "BFDECE");
    //cout << r << endl;

    //    
    //vector<int> nums = { 6, 7, 8, 9, 10, 10, 13};
    //int target = 18;
...
//vector<int> nums = { 1,2,2,2,3, 5, 2};
//
//    cout << solution.findUnsortedSubarray(nums) << endl;
    //string s = "abc3[d]2[ef3[g]2[h]]ij";
    ////string s = "abc3[d]";
    //cout << solution.decodeString(s) << endl;
	//vector<vector<int>> data = { {3, 6, 9, 12, 17, 22},
	//							{5, 11, 11, 16, 22, 26},
	//							{10, 11, 14, 16, 24, 31},
	//							{10, 15, 17, 17, 29, 31},
	//							{14, 17, 20, 23, 34, 37},
	//							{19, 21, 22, 28, 37, 40},
	//							{22, 22, 24, 32, 37, 43} };
    //   cout << solution.searchMatrix(data, 3);
